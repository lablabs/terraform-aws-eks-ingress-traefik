...
variable "namespace" {
  ...
  description = "The K8s namespace in which the traefik service account has been created"
}

variable "settings" {
  type        = map(any)
  default     = {}
  description = "Additional helm sets which will be passed to the Helm chart values, see https://hub.helm.sh/charts/stable/traefik"
}

variable "values" {
  type        = string
  default     = ""
  description = "Additional yaml encoded values which will be passed to the Helm chart, see https://artifacthub.io/packages/helm/traefik/traefik"
}
